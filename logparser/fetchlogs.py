...
def fetch_logs(log_path=None, log_url=None):
    ...
    try:
        ...
        print "Json file not present making from scratch", err
...
def read_url_log(log_dict, log_url):
    ...
    if log_id not in log_dict:
        log_dict[log_id] = {'date': log_file.info()['Last-Modified'],
...
def read_path_log(log_dict, log_file_path):
    ...
    if log_id not in log_dict:
        log_dict[log_id] = {'date': time.ctime(os.stat(log_file_path).st_mtime),
